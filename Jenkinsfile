pipeline {
    ...
    tools {
        maven 'mvn' // Для сборки бэкенда нужен Maven
        jdk 'JDK16' // И Java Developer Kit нужной версии
        nodejs 'njs' // А NodeJS нужен для фронта
    }
    ...
    stages {
        ...
        stage('Send notification') {
            steps {
                sh(""" curl -d '{"text":"Александр Оганов собрал приложение."}' -H 'Content-type: application/json' -X POST https://hooks.slack.com/services/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 """)
            }
        }
    }
}